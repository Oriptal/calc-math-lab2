...
public:
  ...
  Q_INVOKABLE QVariantMap integrate(qint32 functionId,
    ...
    const std::array<IntegrationMethod, 5> methods = {
        IntegrationMethod::LeftRect,   IntegrationMethod::RightRect,
        IntegrationMethod::MidRect,    IntegrationMethod::Trapezoid,
        IntegrationMethod::Simpson,
    };

    QVariantList list;
    QString overallStatus = "ok";
    bool anyPrincipalValue = false;

    for (IntegrationMethod method : methods) {
      IntegrationResult r =
          Integrators::integrate(method, f, a, b, eps, disc);
      QVariantMap item;
      item.insert("method", methodName(method));
      ...
      if (r.status == "ok_principal_value") {
        anyPrincipalValue = true;
      }
      if (r.status == "diverges" || r.status == "max_iter" ||
          r.status == "error" || r.status == "indeterminate") {
        overallStatus = QString::fromStdString(r.status);
      }
    }

    if (overallStatus == "ok" && anyPrincipalValue) {
...
private:
  ...
  static QString methodName(IntegrationMethod method) {
    switch (method) {
    case IntegrationMethod::LeftRect:
      return QStringLiteral("Левые прямоугольники");
    case IntegrationMethod::RightRect:
      return QStringLiteral("Правые прямоугольники");
    case IntegrationMethod::MidRect:
      return QStringLiteral("Средние прямоугольники");
    case IntegrationMethod::Trapezoid:
      return QStringLiteral("Трапеции");
    case IntegrationMethod::Simpson:
      return QStringLiteral("Симпсон");
    }
  ...
  static bool isIntegrandDefined(qint32 functionId, double a, double b) {
    switch (functionId) {
    case 4:
      // 1/√x требует x > 0 на всём отрезке (кроме левой границы, где можно
      // использовать несобственный интеграл 2-го рода).
      if (a < 0.0) {
        return false;
      }
      return true;
    case 3:
      // 1/x — только разрыв в 0, в остальном определена.
      return true;
    default:
      (void)a;
      (void)b;
