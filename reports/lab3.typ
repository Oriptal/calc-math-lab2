...
Порядок точности: $O(h^4)$. На полиномах степени не выше трёх метод даёт точный результат.

#figure(image("resources/flowcharts/simpson.png", width: 60%), caption: [Блок-схема метода Симпсона])

=== Правило Рунге
Эмпирическая оценка погрешности квадратурной формулы порядка $k$ через результаты с шагами $h$ и $h\/2$:
$ R approx abs(I_(h\/2) - I_h) / (2^k - 1) $
где $k = 2$ для прямоугольников и трапеций, $k = 4$ для Симпсона.
...
Заголовочный файл `calc/Integrators.hpp` с базовым классом `Integrator` (правило Рунге и обработка несобственных интегралов) и пятью наследниками-квадратурами:

#raw(read("../calc/Integrators.hpp"), lang: "cpp", block: true)

Реализация (`calc/integrators.cpp`) — собственно квадратурные формулы, правило Рунге, разбиение по точкам разрыва, односторонние пределы и главное значение Коши:
